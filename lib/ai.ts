...
const JSON_SCHEMA_TEMPLATE = `{
  ...
  "aesthetic_descriptor": "ONE short phrase capturing only the SOFT/AESTHETIC dimension of the brief — vibe, mood, era, season, occasion. NO brand. NO color. NO garment type. NO fabric. NO silhouette. This phrase is what FashionCLIP encodes after Algolia has already filtered by the hard literal terms (brand/color/garment/category), so anything literal is wasted gravity that stops it from differentiating already-filtered candidates. Examples: 'blue Khaite dress for summer' -> 'summery breezy unhurried'. 'y2k party' -> 'y2k early 2000s clubby'. 'old money cream cable knit' -> 'old money quiet luxury preppy'. 'dad core wide leg trouser' -> 'dad core unhurried 90s skater'. 4-8 words. Lowercase. No punctuation except commas. If the brief has no clear soft/aesthetic dimension separate from the literal terms, write a 2-3 word mood (e.g. 'minimalist neutral').",
  "aesthetic_descriptor_alts": ["1-2 paraphrases of aesthetic_descriptor expressing the SAME vibe in different words. Same rules: no brand, no color, no garment, no fabric. Helps the FashionCLIP encoder land in a robust region of latent space — single-phrase queries can land in thin areas. Example for 'summery breezy unhurried' -> ['warm-weather light easy', 'sun-drenched relaxed daytime']. Example for 'y2k early 2000s clubby' -> ['nightlife glitter rhinestone party', 'low-rise butterfly metallic']. Same lowercase, comma-only punctuation rule."],
  "visual_signature": "ONE TIGHT sentence (12-18 words MAX) describing what the IDEAL PRODUCT IMAGE should LOOK like. THIS IS DIFFERENT FROM aesthetic_descriptor — that field is abstract vibe; this field is concrete visuals encoded by FashionCLIP. CRITICAL CONSTRAINTS: (1) ONE garment only — DO NOT mention what it's 'paired with' or 'styled over'; FashionCLIP encodes the whole sentence and outfit-shaped signatures dilute the per-image cosine across two garments, hurting retrieval. (2) 12-18 words. Tight, telegraphic, FashionCLIP-vocabulary. Long signatures exceed CLIP's effective context and the encoder averages over noise. (3) Pack visual specs DENSELY: silhouette + fabric/texture + ONE embellishment or cut detail. Think product-photography caption, not editorial prose. (4) MUST NOT restate brand/color the user explicitly typed (Algolia already filters those — restating wastes specificity). DO infer visual attributes from abstract briefs (y2k -> describe what y2k pieces actually look like). Lowercase, prefix with 'a photo of'. Examples: 'y2k party' -> 'a photo of a tight strappy metallic minidress with low-rise hem and thin halter straps'. 'old money cream cable knit' -> 'a photo of a chunky cream cable-knit sweater with a relaxed turtleneck and ribbed cuffs'. 'dad core wide leg trouser' -> 'a photo of pleated wide-leg trousers in washed cotton with a relaxed drape and tapered hem'. 'blue Khaite dress for summer' -> 'a photo of a relaxed midi dress in breezy linen with a soft a-line silhouette'. 'cottagecore picnic' -> 'a photo of a flowy floral midi dress with puffed sleeves and eyelet or smocked detailing'. 'streetwear hoodie black' -> 'a photo of an oversized cotton hoodie with dropped shoulders and a centered kangaroo pocket'. NEGATIVE EXAMPLE — do NOT do this: 'a photo of a chunky sweater paired with tailored trousers or a midi skirt' (that's two garments — the encoder averages and retrieval suffers).",
  "visual_signature_alts": ["1-2 paraphrases of visual_signature — same SINGLE garment, same visual specs, expressed in slightly different vocabulary. Stage 2 encodes all of (signature + alts), averages the unit vectors, and uses the ensemble as the rerank query — robust to any one phrasing landing in a thin region of CLIP latent space. Same constraints as visual_signature: ONE garment, 12-18 words, lowercase, prefix with 'a photo of', no brand/color the user explicitly typed. Just rephrase silhouette/fabric/detail using synonyms. Example for 'y2k party' signature 'a photo of a tight strappy metallic minidress with low-rise hem and thin halter straps' -> ['a photo of a fitted shimmer minidress with low waist, slim spaghetti straps, and body-hugging cut', 'a photo of a short metallic-finish dress with thin shoulder ties, snug bodice, and hip-skimming hem']. Example for 'old money cream cable knit' signature 'a photo of a chunky cream cable-knit sweater with a relaxed turtleneck and ribbed cuffs' -> ['a photo of a thick ivory cable-stitch knit pullover with a roll neck and ribbed hem', 'a photo of an oversized cream wool cableknit jumper with a high neckline and textured weave']."],
