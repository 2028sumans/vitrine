...
const JSON_SCHEMA_TEMPLATE = `{
  ...
  "visual_signature": "ONE concrete visual sentence describing what the IDEAL PRODUCT IMAGE should LOOK like. THIS IS DIFFERENT FROM aesthetic_descriptor — that field is abstract vibe; this field is concrete visuals. Written in FashionCLIP's native 'a photo of ...' vocabulary so the encoder lands in a dense region of latent space (abstract phrases like 'clubby' land in thin regions and produce fuzzy cosines). MUST INCLUDE: silhouette, fabric or texture, ≥1 embellishment or detail when applicable, length or cut. MUST NOT restate the brand or color the user explicitly typed (Algolia already filters on those — restating wastes the FashionCLIP query's specificity). DO respect inferred attributes for anchor-poor briefs (e.g. y2k -> describe what y2k pieces actually look like). 1-2 sentences, lowercase, prefix with 'a photo of'. Examples: 'y2k party' -> 'a photo of a tight strappy minidress in metallic or rhinestone fabric with a low-rise hem and a halter or one-shoulder neckline'. 'old money cream cable knit' -> 'a photo of a chunky cable-knit sweater in cream with ribbed cuffs and a relaxed turtleneck or crewneck silhouette'. 'dad core wide leg trouser' -> 'a photo of pleated wide-leg trousers in washed cotton or wool with a relaxed drape and a clean tapered hem'. 'blue Khaite dress for summer' -> 'a photo of a relaxed midi dress with a breezy linen or poplin texture and a soft a-line silhouette'. 'cottagecore picnic' -> 'a photo of a floral midi dress with puff sleeves and eyelet or smocked detailing in soft pastel tones'.",
