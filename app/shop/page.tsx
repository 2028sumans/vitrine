...
function ShopPageContent() {
  ...
  const refreshBiasedAhead = useCallback(async () => {
    ...
    try {
      ...
      // Two splice strategies depending on the active view:
      //
      //   Scroll view (TikTok-style): we know the user's active card index
      //   from the IntersectionObserver, so we splice biased items in just
      //   past the active card (+ 2). The current card and the next stay
      //   stable so swiping doesn't visibly jolt; the second-next card and
      //   onward become biased.
      //
      //   Grid view: there's no active-index concept (the user could be
      //   scrolled anywhere). Splicing-from-2 would tear up the grid
      //   visually. Instead we APPEND the biased batch to the bottom — the
      //   current grid stays stable, the user scrolls naturally and the
      //   biased items land in the next chunk they reach.
      const isGrid = viewModeRef.current === "grid";
      setProducts((prev) => {
        if (isGrid) {
          const prevIds = new Set(prev.map((p) => p.objectID));
          const freshUnseen = fresh.filter((p) => !prevIds.has(p.objectID));
          freshUnseen.forEach((p) => seenIdsRef.current.add(p.objectID));
          return [...prev, ...freshUnseen];
        }
      ...
      // (Skip in grid mode — appended items are treated as fresh page-0 fill;
      // moving page forward would skip ahead and break dedupe-against-seen.)
      if (!isGrid) {
        setPage(1);
        setHasMore(true);
      }
      ...
      <main className="flex-1 pt-20 pb-24 px-8 max-w-7xl mx-auto w-full">
        ...
        {(isCategoryMode || isAllMode) && (
          <section
            className={
              tasteSearchActive
                ? "mb-6"                            // results: full-width, no frame
                : "mb-6 border border-border-mid"   // intake: panel chrome
            }
          >
            <TasteShopFlow
...
function GridTile({
  ...
  return (
    // Wrapper is `group relative` so the heart can absolute-position over
    // the image and read group-hover state. The card itself is still a
    // single click-through to the brand site.
    <div className="group relative">
      ...
      {/* Heart toggle — sits on the image, top-right. Filled when saved.
          Without this, grid view collected zero preference signal and the
          adaptation pipeline (refreshBiasedAhead → CLIP centroid pull from
          Pinecone) never fired. Now a heart in grid view feeds the same
          handleSave path as the scroll view's save: shortlist update, click
          history bump, and a fresh /api/shop-all call with bias + likes that
          splices similar-from-the-whole-catalog items into the queue. */}
      <button
        ...
        className="absolute top-2 right-2 z-10 w-9 h-9 rounded-full bg-background/90 border border-border-mid flex items-center justify-center text-foreground/70 hover:text-foreground hover:border-foreground/60 opacity-0 group-hover:opacity-100 focus:opacity-100 transition-opacity duration-200"
        style={isSaved ? { opacity: 1, color: "#2A3316" } : undefined}
      >
        <svg
          viewBox="0 0 24 24"
          className="w-4 h-4"
